...
class SlicePainter extends CustomPainter {
  ...
  void _drawBlade(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < pointsList.length; i++) {
      ...
      final double newXLeft = x1 - normalizedVectorY * (i / pointsList.length * distance);
      final double newYLeft = y1 + normalizedVectorX * (i / pointsList.length * distance);

      final double newXRight = x1 - normalizedVectorY * (i / pointsList.length * -distance);
      final double newYRight = y1 + normalizedVectorX * (i / pointsList.length * -distance);
